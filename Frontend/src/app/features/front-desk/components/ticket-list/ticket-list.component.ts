...
export class TicketListComponent implements OnInit {
  ...
  fetch(): void {
    ...
    switch (this.type()) {
      case 'housekeeping':
        request$ = forkJoin([
          this.hkApi.getAll({ status: 'Pending', pageSize: 200 }).pipe(
            map(r => r.data),
            catchError(() => of([] as any[]))
          ),
          this.hkApi.getAll({ status: 'InProgress', pageSize: 200 }).pipe(
            map(r => r.data),
            catchError(() => of([] as any[]))
          ),
        ]).pipe(map(([p, ip]) => [...(p || []), ...(ip || [])]));
      ...
      case 'maintenance':
        request$ = forkJoin([
          this.mtApi.getAll({ status: 'Pending', pageSize: 200 }).pipe(
            map(r => r.data),
            catchError(() => of([] as any[]))
          ),
          this.mtApi.getAll({ status: 'InProgress', pageSize: 200 }).pipe(
            map(r => r.data),
            catchError(() => of([] as any[]))
          ),
        ]).pipe(map(([p, ip]) => [...(p || []), ...(ip || [])]));
      ...
      case 'foodOrder':
        request$ = forkJoin([
          this.orderApi.getAll({ status: 'Pending', pageSize: 200 }).pipe(
            map(r => r.data),
            catchError(() => of([] as any[]))
          ),
          this.orderApi.getAll({ status: 'Preparing', pageSize: 200 }).pipe(
            map(r => r.data),
            catchError(() => of([] as any[]))
          ),
        ]).pipe(map(([p, ip]) => [...(p || []), ...(ip || [])]));
