<!-- explicit-requirements-protocol:start -->

# Explicit Requirements Protocol

This model does not proactively ask clarifying questions. Ambiguity gets silently
resolved with an assumption rather than surfaced. This section exists to force
assumptions into the open before code is written.

## Always Do

- **MUST produce a written plan before any edit**, containing:
  1. **Interpretation** — a one-paragraph restatement of the task in your own words.
  2. **Assumptions** — an explicit bullet list of every non-stated decision you are
     about to make (library choice, config location, naming, error handling
     strategy, where new code lives in the architecture, auth/secrets handling).
  3. **Acceptance criteria** — a checklist of what "done" means, derived from the
     task and the assumptions above.
  4. **Edit surface** — the exact files/symbols you intend to touch (from the
     GitNexus + Serena planning pass), and nothing else.
- **MUST pause and present the plan to the user before editing** whenever any
  assumption in step 2 is security-relevant, changes a public contract (API
  route, DB schema, DI registration visible to other modules), or introduces a
  new external dependency or third-party service call. Wait for explicit
  confirmation before proceeding.
- For assumptions that are low-stakes and reversible (variable names, internal
  helper structure, file layout within a new feature folder), state them in the
  plan but proceed without waiting — do not block on trivial decisions.
- **MUST default to the most conservative, least-surprising option** when an
  assumption has to be made without user input: match existing project
  conventions over introducing new ones, prefer configuration over hardcoding,
  prefer feature flags/toggles for anything user-facing and new.
- **MUST restate the acceptance criteria at the end of the task** as a checklist
  with each item marked done/not-done, so completion can be verified against
  what was actually agreed, not just what was built.

## Never Do

- NEVER silently choose between two materially different implementation
  approaches (e.g. streaming vs. polling, sync vs. async provider call,
  session-scoped vs. app-scoped client) without listing it as an assumption the
  user can override.
- NEVER treat an underspecified prompt as license to expand scope — if
  something is unclear, assume the narrowest reasonable interpretation and say
  so, don't build the maximal version "to be safe."

<!-- explicit-requirements-protocol:end -->

<!-- gitnexus:start -->
# GitNexus — Code Intelligence

This project is indexed by GitNexus as **resort-operations-management** (8691 symbols, 23922 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/resort-operations-management/context` | Codebase overview, check index freshness |
| `gitnexus://repo/resort-operations-management/clusters` | All functional areas |
| `gitnexus://repo/resort-operations-management/processes` | All execution flows |
| `gitnexus://repo/resort-operations-management/process/{name}` | Step-by-step execution trace |
