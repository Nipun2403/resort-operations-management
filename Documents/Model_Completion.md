...
--------------
✓ src/app/shared/components/generic-crud/generic-crud.component.ts
  - Declared edit = output<any>() output parameter
  - Updated openEditModal(row) to call edit.emit(row)
✓ src/app/features/admin/pages/management/room-management.component.ts
  - Implemented onEdit(entity: Room) method to set the editingEntity signal
✓ src/app/features/admin/pages/management/room-management.component.html
  - Bound (edit)="onEdit($event)" event handler on <app-generic-crud> element

REQUIREMENTS IMPLEMENTED
------------------------
✓ Edit Output on Generic CRUD
  - Declared output to communicate currently edited entity to parent components
  - Correctly emits event just before edit dialog is presented
✓ Parent Integration
  - Parent component subscribes to the edit event via Angular template event binding
  - Stores editingEntity signal correctly so that the onSave save handler determines the correct flow
✓ API Call Resolution
  - Confirmed that saving an edit now properly triggers update (PATCH /api/v1/rooms/{id}) instead of create (POST /api/v1/rooms)
...
LOGIC TRACES
------------
Flow: Room Edit Save Flow
  Entry: Click edit button on Room row -> change a value -> click save
  Path: User clicks Edit -> openEditModal(row) executes -> emits edit event with row -> parent stores row in editingEntity -> user edits & submits -> save event is emitted with form value -> parent detects editingEntity is non-null -> calls RoomApiService.update(id, dto) -> PATCH /api/v1/rooms/{id} is executed
  Result: ✓ Matches spec (properly executes PATCH request instead of POST request)
